...
/*__________以下根据.h文件的定义相应修改_________________*/

//内存池(32字节对齐)（真正储存东西的地方）
__align(32) u8 mem1base[InrRAM_MAX_SIZE];															//内部SRAM内存池

__align(32) u8 mem2base[ExRAM1_MAX_SIZE] __attribute__((at(SRAM1_BANK3_ADDR)));					    //外部SRAM内存池，注意定义好基地址

__align(32) u8 mem3base[InrCCM_MAX_SIZE] __attribute__((at(0X10000000)));							//内部CCM内存池，注意定义好基地址
